//
//  AnimeEntryDetailBridgeV2_8_1.swift
//  AniShelf
//
...
extension SchemaV2_8_1.AnimeEntryDetail {
    public func snapshotDTO() -> AnimeEntryDetailDTO {
        AnimeEntryDetailDTO(
            ...
            productionCompanies: orderedProductionCompanies.map {
    ...
    public convenience init(from dto: AnimeEntryDetailDTO) {
        self.init(
            ...
            productionCompanies: Self.makeProductionCompanies(from: dto.productionCompanies),
            characters: Self.makeCharacters(from: dto.characters),
            staff: Self.makeStaff(from: dto.staff),
            seasons: dto.seasons.map(SchemaV2_8_1.AnimeEntrySeasonSummary.init(from:)),
    ...
    public convenience init(fromLegacy payload: LegacyAnimeEntryDetailPayload) {
        self.init(
            ...
            staff: [],
            seasons: payload.seasons.map(SchemaV2_8_1.AnimeEntrySeasonSummary.init(fromLegacy:)),
    ...
    public func apply(dto: AnimeEntryDetailDTO) {
        ...
        episodeCount = dto.episodeCount
        seasonCount = dto.seasonCount
        replaceProductionCompanies(with: dto.productionCompanies)
    ...
    private func replaceProductionCompanies(with dtos: [AnimeEntryProductionCompanyDTO]) {
        if let modelContext {
            for company in productionCompanies {
                modelContext.delete(company)
            }
        }
        productionCompanies = Self.makeProductionCompanies(from: dtos)
        productionCompanies.forEach { $0.detail = self }
    }
    ...
    private func replaceSeasons(with dtos: [AnimeEntrySeasonSummaryDTO]) {
        ...
        seasons = dtos.map(SchemaV2_8_1.AnimeEntrySeasonSummary.init(from:))
    ...
    ) -> [SchemaV2_8_1.AnimeEntryCharacter] {
        dtos.enumerated().map {
            SchemaV2_8_1.AnimeEntryCharacter(from: $0.element, displayOrder: $0.offset)
        }
    }

    private static func makeProductionCompanies(
        from dtos: [AnimeEntryProductionCompanyDTO]
    ) -> [SchemaV2_8_1.AnimeEntryProductionCompany] {
        dtos.enumerated().map {
            SchemaV2_8_1.AnimeEntryProductionCompany(
                from: $0.element,
    ...
    private static func makeCharacters(
        ...
        payloads.enumerated().map {
            SchemaV2_8_1.AnimeEntryCharacter(fromLegacy: $0.element, displayOrder: $0.offset)
        }
    }

    private static func makeStaff(
        from dtos: [AnimeEntryStaffDTO]
    ) -> [SchemaV2_8_1.AnimeEntryStaff] {
        dtos.enumerated().map {
            SchemaV2_8_1.AnimeEntryStaff(from: $0.element, displayOrder: $0.offset)
        }
    }

    private static func makeEpisodes(
        from dtos: [AnimeEntryEpisodeSummaryDTO]
    ) -> [SchemaV2_8_1.AnimeEntryEpisodeSummary] {
        dtos.enumerated().map {
            SchemaV2_8_1.AnimeEntryEpisodeSummary(from: $0.element, displayOrder: $0.offset)
        }
    }
    ...
    private static func makeEpisodes(
        ...
        payloads.enumerated().map {
            SchemaV2_8_1.AnimeEntryEpisodeSummary(fromLegacy: $0.element, displayOrder: $0.offset)
        }
    }
}
