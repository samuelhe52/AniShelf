...
extension InfoFetcher {
    ...
    func animeTVSeriesInfo(tmdbID: Int, language: Language) async throws -> EntryMetadata? {
        ...
        guard payload.series.genres?.contains(where: { $0.id == 16 }) == true else {
            return nil
        }
    ...
    private func tvSeriesDetail(
        ...
        AnimeEntryDetailDTO(
            ...
            runtimeMinutes: Self.runtimeMinutes(from: series.episodeRunTime?.first),
    ...
    private func makeSeasonSummaries(
        ...
        seasons.map {
            AnimeEntrySeasonSummaryDTO(
                ...
                title: $0.name,
                posterPath: TMDbImagePath.storagePath(from: $0.posterPath),
